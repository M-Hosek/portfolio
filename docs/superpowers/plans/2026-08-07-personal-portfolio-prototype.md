...
- **⚠️ The shipped `generative.js` intentionally differs from Task 8's code block above.** Task 8's review found a real defect in the code as planned: the `IntersectionObserver` and the `visibilitychange` listener were registered only inside the `else` branch taken when reduced motion is off at load. A user who loaded the page with `prefers-reduced-motion: reduce` enabled and then turned it off mid-session got an animation with **no off-screen pause and no hidden-tab pause** — an unpausable loop burning CPU in a background tab, with `onScreen` still stuck at its initial `true`. The fix (approved by the repository owner, commit `22eafe4`) hoists both registrations out of the `else` so they always run; they are already inert under reduced motion because `start()` guards on `reduceMotion.matches` independently. The reduced-motion guarantee — exactly one static frame, zero `requestAnimationFrame` calls — was verified to survive the change. **Treat the shipped file, not the code block above, as correct.**
